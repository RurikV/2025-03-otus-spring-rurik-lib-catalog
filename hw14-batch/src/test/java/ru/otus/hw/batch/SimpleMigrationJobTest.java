...
class SimpleMigrationJobTest {
    ...
    @Test
    void shouldCompleteFullMigrationJobWithTestData() throws Exception {
        System.out.println("[DEBUG_LOG] Testing complete migration job with test data");
        
        // Given - Setup test data in MongoDB
        setupMongoTestData();
    ...
    void shouldHandleEmptyMongoDatabase() throws Exception {
        ...
        // Given - MongoDB is already empty from setUp()
        JobParameters jobParameters = new JobParametersBuilder()
                .addLong("timestamp", System.currentTimeMillis())
                .toJobParameters();
    ...
    @Test
    void shouldHandleJobRestart() throws Exception {
        System.out.println("[DEBUG_LOG] Testing job restart functionality");
        
        // Given - Setup test data and run job first time
        setupMongoTestData();
        ...
        JobExecution firstExecution = jobLauncher.run(migrationJob, firstJobParameters);
        assertThat(firstExecution.getStatus()).isEqualTo(COMPLETED);
        
        // Clear mappings to simulate restart
        idMappingService.clearMappings();
        
        // When - Run job again with different parameters
        ...
        JobExecution secondExecution = jobLauncher.run(migrationJob, secondJobParameters);

        // Then
        assertThat(secondExecution.getStatus()).isEqualTo(COMPLETED);
        
        // Verify data is still consistent after restart
